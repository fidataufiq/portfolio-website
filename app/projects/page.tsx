  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      if (activeFilter === "Design") {
        return category.includes("design") || category.includes("ui/ux") || project.technologies.some((technology) => technology.toLowerCase().includes("figma") || technology.toLowerCase().includes("ui/ux"));
      }

      if (activeFilter === "Web") {
        return category.includes("web") || project.technologies.some((technology) => technology.toLowerCase().includes("next.js") || technology.toLowerCase().includes("javascript") || technology.toLowerCase().includes("codeigniter"));
      }

      if (activeFilter === "Application") {
        return category.includes("application");
      }
        ...
        <Container>
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-xs font-medium transition-all duration-300 ${
                        isActive ? "bg-white text-black shadow-lg shadow-white/10" : "border border-neutral-800 bg-white/[0.02] text-neutral-500 hover:-translate-y-0.5 hover:border-neutral-600 hover:bg-white/[0.05] hover:text-neutral-200"
                      }`}
                    >
                      {filter}
                    </button>
          ...
          {filteredProjects.length === 0 && (
            <div className="mt-10 rounded-2xl border border-dashed border-neutral-800 bg-white/[0.02] px-6 py-16 text-center">
              ...
              <button type="button" onClick={() => setActiveFilter("All")} className="mt-6 rounded-full border border-neutral-700 px-4 py-2 text-xs font-medium text-neutral-400 transition hover:border-neutral-500 hover:text-white">
                View all projects
              </button>
            </div>
          )}
